...
def main() -> int:
    ...
    # 問題 CSV の category（site-config fields）向けハブ — 用語未登録分野もリンク切れ防止
    seen_hubs = {field_hub_slug(cat) for cat in by_cat}
    for field in fields():
        name = str(field.get("name") or "").strip()
        if not name:
            continue
        hub = field_hub_slug(name)
        if hub in seen_hubs:
            continue
        seen_hubs.add(hub)
        hub_dir = TERMS_DIR / hub
        hub_dir.mkdir(parents=True, exist_ok=True)
        (hub_dir / "index.html").write_text(
            build_field_hub_html(name, hub, by_cat.get(name, []), base),
            encoding="utf-8",
        )
        hub_count += 1
